...
We are proccessing only UDP packets between DMRGateway and [gw_hytera_mmdvm](http://ham-dmr.at/?wpfb_dl=651) and between [gw_hytera_mmdvm](http://ham-dmr.at/?wpfb_dl=651) and the repeater itself, depend on which ports are defined.
...
4. Voice_Start_HYTERA maybe not complete too - replace with payload from similar/first mmdvm-packet which starts the transmission (fixed)
